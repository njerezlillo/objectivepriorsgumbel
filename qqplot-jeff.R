...
Generator <- function (index) {
  ...
  aux_ecfd <- ecdf(x)
  quant <- function(z) mean(vmu) - mean(vvarp) * log(-log(z))
  xp <- sapply(c(0.50, 0.75, 0.90, 0.95, 0.975), quant)
  
  qqplot(x, quant(aux_ecfd(x)), xlab = "Empirical data", 
         ylab = "Fitted data", col = "red", 
         main = month[index])
  abline(0,1)
}
